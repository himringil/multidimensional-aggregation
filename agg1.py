    ...
    class TimeSeries(AggTreeBase.TimeSeriesBase):
        ...
        def delete_zero_elements(self):
            for key in list(self.queue.keys()):
                ...
                if f == 'count' and sum([el.value for el in self.queue[key] if el.value is not None]) == 0:
        ...
        def _merge_trees(self, nodes_to, node_from):
            for node_to in nodes_to:
                if node_to.name == node_from.name:
                    node_to.value = self._new_value(node_to.name, node_to.value, node_from.value)
        ...
        def add(self, dt: datetime, values):
            ...
            if not self.time_start:
                for el in values:
                    f = self._get_func(el)
                    self.queue[el] = [self.ValuesNode(f'{f if f == "count" else el}', f'{f if f == "count" else el}', None) for i in range(self.q)]
            ...
            while not self.time_start + self.time_range - self.time_delta <= dt < self.time_start + self.time_range:
                ...
                for el in self.queue:
                    ...
                    if value.value:
                        old_values[el] = value
                    f = self._get_func(el)
                    self.queue[el].append(self.ValuesNode(f'{f if f == "count" else el}', f'{f if f == "count" else el}', 0 if f == 'count' else None))
