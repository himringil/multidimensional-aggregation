...
def modify_node(node: TimeSeries, dt: datetime, values, graph):
    for _values, _time_start, _graph in node.add_element(dt, values, graph):
        # insert it to childs
        for child in node.children:
            modify_node(child, _time_start, _values, _graph)
            ...
            if filename.endswith('.parquet'):
                ...
                for index, row in df.iterrows():
                    if not row['src']:
                        continue
                    try:
                        datetime, values, graph = select_params(row, params)
                        modify_node(tree, datetime, values, graph)
